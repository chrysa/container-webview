...
class TestStreamLogsWebSocket:
    ...
    def test_closes_4001_when_token_invalid(self):
        ...
        with (
            ...
            client.websocket_connect("/api/logs/myproject/services/web/logs?token=invalidtoken") as ws,
        ):
            ws.receive_text()
    ...
    def test_closes_4004_when_project_not_found(self, mocker, test_token):
        ...
        with (
            TestClient(app) as client,
            pytest.raises(WebSocketDisconnect),
            client.websocket_connect(f"/api/logs/missing/services/web/logs?token={test_token}") as ws,
        ):
            ws.receive_text()
    ...
    def test_closes_4004_when_container_not_found(self, mocker, test_token):
        ...
        with (
            TestClient(app) as client,
            pytest.raises(WebSocketDisconnect),
            client.websocket_connect(f"/api/logs/myproject/services/web/logs?token={test_token}") as ws,
        ):
            ws.receive_text()
